...
function migratePayloadToCurrentSchema(payload: unknown): { ok: boolean; preferences: unknown; reason: string } {
  ...
  const parsed = payload as { schema?: unknown; preferences?: unknown; prefs?: unknown };
  ...
  if (Number.isInteger(schema) && BUILD_111_SUPPORTED_SCHEMA_VERSIONS.has(schema as number)) {
    if (parsed.prefs && typeof parsed.prefs === 'object') {
      return { ok: true, preferences: parsed.prefs, reason: 'Preset migrated from legacy schema.' };
    }
    return { ok: false, preferences: null, reason: 'Preset payload is invalid.' };
  }

  // Legacy wrapper without explicit schema occasionally appears in copied payloads.
  if (parsed.prefs && typeof parsed.prefs === 'object' && !Number.isInteger(schema)) {
    return {
      ok: true,
      preferences: parsed.prefs,
